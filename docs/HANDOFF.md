...
本文用于新的 Codex 窗口快速恢复 Story Forge 当前项目上下文。请以当前 Git 代码和 `docs/` 文档为优先事实来源；`AGENTS.md` 和 `PROJECT_STATE.md` 可辅助理解历史背景，但如与当前代码或本文档不一致，以当前项目实现为准。不要凭历史对话补全事实。
...
当前 Dashboard 已落地的页面结构为：

- 开始创作：用户启动每日创作的入口。旧文档中也称为“今日创作”，当前 UI 文案以“开始创作”为准。
- 推荐主题：展示今日推荐主题，支持刷新主题和用指定主题生成任务。
- 任务队列：展示最近任务，支持单任务删除/取消和一键清空。
- 作品库：从本地 `stories/` 文件夹读取已完成作品，支持分页、详情查看、指标保存和打开文件夹。

当前代码中的左侧导航实际为：`开始创作`、`推荐主题`、`任务队列`、`作品库`。

后续规划但当前未落地为独立导航的模块：

- 数据复盘：未来用于展示阅读量、流失、题材表现等复盘结果。
- 系统设置：未来用于承载 provider、模型、路径和运行模式设置。

新窗口开发时不要把“数据复盘”和“系统设置”当成已完成页面；如需新增，应另行明确需求，并保持现有数据源和架构约束。
...
- 数据复盘模块尚未作为独立导航页落地。
- 系统设置模块尚未作为独立导航页落地。
- 历史文档可能保留旧阶段描述；遇到差异时，以当前 Git 代码和 `docs/` 当前文档为准。
