...
V1.5 的文档骨架仍然保留。M2 起项目已经新增可运行 Workflow Engine，M3 起新增最小网页入口；默认仍使用 mock provider，只有显式配置后才调用真实 OpenAI API。
...
- 不自动发布。
- 不绕过任何网站风控。
- 默认不调用 AI API；只有设置 `STORY_FORGE_LLM_PROVIDER=openai` 或运行 `--provider=openai`，并提供 `OPENAI_API_KEY` 后才会调用真实 OpenAI API。
...
M3.1 默认不调用真实 AI API；若启动前设置 `STORY_FORGE_LLM_PROVIDER=openai` 和 `OPENAI_API_KEY`，点击“开始今天创作”会通过 Execution Architecture 调用 OpenAI。M3.1 不接入发布平台，不开发用户系统、复杂图表、prompt 编辑器或 metrics 分析页。

## Real AI Execution Architecture

当前执行架构采用 mock → real 渐进升级。默认使用 mock provider，显式切换后可调用真实 OpenAI Responses API。
...
不要把 `OPENAI_API_KEY` 写入仓库文件；`.env*` 已被 `.gitignore` 忽略。
...
Agent Runtime
LLM Provider
Mock LLM / OpenAI Responses API
```

本阶段新增 execution trace、agent input/output log 和 pipeline state tracking。OpenAI Provider 已接入；Claude Provider 仍是占位，不会调用外部 API。
