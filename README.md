...
V1.5 仍然不包含任何真实执行代码。所有“运行”都是流程约定、文件契约和 agent 职责说明。
...
- 不自动发布。
- 不绕过任何网站风控。
- 不写 Python、JavaScript、TypeScript 或其他真实 pipeline 执行代码。
- 不调用 AI API，除非用户在后续版本明确授权真实执行。
...
确认 agent、rules、prompts、planning、templates、scripts、metrics、dashboard 文档都存在且非空；确认没有新增 `.py`、`.js`、`.ts` 执行代码文件。
